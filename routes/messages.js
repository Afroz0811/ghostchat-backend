...
router.get("/:chatId", auth, async (req, res) => {
  try {
    ...
    const messages = await Message.find({ chatId:req.params.chatId, deletedAt:null })
      .populate("from", "username avatar").sort({ createdAt:-1 }).skip((page-1)*limit).limit(limit);
    const decrypted = messages.reverse().map(msg => {
      const m = msg.toObject();
      try { m.text = decrypt(m.text, m.iv); } catch { m.text = "🔒 Could not decrypt"; }
      delete m.iv; return m;
    });
...
router.delete("/:id", auth, async (req, res) => {
  try {
    ...
    msg.deletedAt = new Date(); msg.text = ""; await msg.save();
